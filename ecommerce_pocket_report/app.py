...
def main():
    ...
    if models:
        # Calculate optimal number of columns (up to 4 per row)
        # If more than 4 models, we might want a grid, but for now we'll just expand columns
        # or limit to 4 if we want to wrap.
        # But Streamlit columns are strictly side-by-side. 
        # If we have many models, we should probably use a grid loop.
        # Given the "Pocket Report" nature, usually it's 3-5 models.
        # Dynamic columns based on count is safest to prevent IndexError.
        num_cols = len(models)
        cols_summary = st.columns(num_cols)
    ...
    if models:
        ...
        # 1. By Color Row
        cols_color = st.columns(num_cols)
        for idx, model_name in enumerate(models):
            with cols_color[idx]:
        ...
        # 3. By State Row
        cols_state = st.columns(num_cols)
        for idx, model_name in enumerate(models):
            with cols_state[idx]:
        ...
        cols_dealer = st.columns(num_cols)
        for idx, model_name in enumerate(models):
